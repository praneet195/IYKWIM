...
def convert_video2audio1(vid_path, qp_pth):
        '''
        serialize video
        ...
        audioObj = wave.open(qp_pth+vidName+".wav", 'wb')
...
if __name__ == "__main__":
    ...
    for qp in qps:
        try:
            qp_pth = sys.argv[1]+qp+"_Wavs/"
            os.makedirs(sys.argv[1]+qp+"_Wavs/")
        except:
            pass
        all_vid_path = sys.argv[1]+qp+"/"
        for vid_path in os.listdir(all_vid_path):
            convert_video2audio1(all_vid_path+vid_path, qp_pth)
